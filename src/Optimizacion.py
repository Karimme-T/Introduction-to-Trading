...
def suggest_params(trial) -> StrategyParams:
    ...
    p = StrategyParams(
        ...
        n_units       = trial.suggest_float("n_units", 0.01, 0.2, step=0.01)
    )
    # coherencia MACD (slow > fast):
    if p.macd_slow <= p.macd_fast:
        p.macd_slow = p.macd_fast + 1
    return p

def objective_factory(train_df: pd.DataFrame, test_df: pd.DataFrame):
    """
    Maximizamos Calmar en TEST para seleccionar hiperparámetros (evita sobreajuste al train).
    """
    def objective(trial):
        p = suggest_params(trial)
        sig_tr = get_signals(train_df, p)
        res_tr = backtest(sig_tr, p)

        # evaluamos en TEST (datos fuera de entrenamiento)
        sig_te = get_signals(test_df, p)
        res_te = backtest(sig_te, p)
        stats  = compute_metrics(res_te)
        # objetivo: Calmar (si -inf o NaN, penalizamos)
...
def optimize_params(train_df: pd.DataFrame, test_df: pd.DataFrame, n_trials: int = 50, study_name: str = "calmar_opt"):
    if optuna is None:
    ...
    study.optimize(objective_factory(train_df, test_df), n_trials=n_trials, show_progress_bar=False)
    return study
# %%
